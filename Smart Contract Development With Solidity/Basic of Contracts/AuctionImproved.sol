...
contract AuctionImproved{
    ...
    mapping(address => uint) public bidderLastBidTime;

    event LogBid(address bidder, uint bad, address highestBidder, uint highestBid);
    ...
    modifier onlyHigherBid {
        require(msg.value >= fundsByBidder[highestBidder] + bidMargin);
        _;
    }

    modifier onlyAfterOneHour{
        require(now > bidderLastBidTime[msg.sender] + 1 hours);
        _;
    }
        ...
        onlyHigherBid
    {
        ...
        bidderLastBidTime[msg.sender] = now;
