    ...
    public class ModifyUserCommand
    {
        ...
        public static string Execute(string[] data)
        {
            ...
            using (var context = new PhotoShareContext())
            {
                ...
                string exceptionMess = $"Value {newValue} not valid." + Environment.NewLine;
                switch (property)
                {
                    case "password":
                        if (!newValue.Any(c => Char.IsLower(c)) || !newValue.Any(c => Char.IsDigit(c)))
                        {
                            throw new ArgumentException(exceptionMess + "Invalid Password!");
                        }
                    ...
                    case "borntown":
                        ...
                        if (bornTown == null)
                        {
                            throw new ArgumentException( exceptionMess + $"Town {newValue} not found!");
                        }
